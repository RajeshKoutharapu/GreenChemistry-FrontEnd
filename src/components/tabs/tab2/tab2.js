...
useEffect(() => {
  setMainInstrumentData(prev =>
    prev.slice(0, mainInstrumentsCount).concat(
      Array.from({ length: Math.max(0, mainInstrumentsCount - prev.length) }, () => '')
    )
  );

  setMainInstrumentTables(prev => {
    const newTables = {};
    for (let i = 0; i < mainInstrumentsCount; i++) {
      newTables[i] = prev[i] || {};
    }
    return newTables;
  });
